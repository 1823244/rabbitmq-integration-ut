...
Функция ЗаполнитьРеквизитыТЧ_РасшифровкаПлатежа(СтруктураОбъекта, ОбъектДанных)
	...
	Для счТовары = 0 По деф.ТЧРасшифровкаПлатежа.Количество()-1 Цикл
		...
		СтрокаТЧ.Организация = ОбъектДанных.организация;//ксп_ИмпортСлужебный.НайтиОрганизацию(деф.Организация, мВнешняяСистема);
		...
		//ЕНС
		Если ЗначениеЗаполнено(СсылкаДоговор) и ЗначениеЗаполнено(СсылкаДоговор.ВерсияДанных) Тогда
			ЗаписьЖурналаРегистрации("Импорт из ЕРП.ПоступлениеБезналичныхДенежныхСредств",
				УровеньЖурналаРегистрации.Информация,,,"Нашли договор в исходных данных, в строке ТЧ");
		...
		Иначе 
			ЗаписьЖурналаРегистрации("Импорт из ЕРП.ПоступлениеБезналичныхДенежныхСредств",
				УровеньЖурналаРегистрации.Информация,,,"НЕ Нашли договор в исходных данных, в строке ТЧ, используем договор из шапки");
					
			СтрокаТЧ.ОбъектРасчетов = НайтиОбъектРасчетов(ОбъектДанных.Договор, 
				Неопределено, 
				ОбъектДанных.Организация, 
				Неопределено, 
				ОбъектДанных.Договор);
...
Функция НайтиОбъектРасчетов(СсылкаНаОбъект, СсылкаПартнер, СсылкаОрганизация, СсылкаКонтрагент, СсылкаДоговор)
	...
	Запрос.УстановитьПараметр("Объект", СсылкаДоговор);
